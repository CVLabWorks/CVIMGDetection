...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, zero_init_residual=False):
        ...
        self.ca1 = ChannelAttention(64)  # 对应conv1的输出通道数
        self.ca2 = ChannelAttention(256) # 对应layer1的输出通道数
    ...
    def forward(self, x):
        ...
        # 新增多尺度NPR
        # NPR_05 = x - self.interpolate(x, 0.5)
        # NPR_025 = x - self.interpolate(x, 0.25)  # 更粗的尺度
        # NPR_075 = x - self.interpolate(x, 0.75)  # 更细的尺度
        # NPR = (NPR_05 * 0.5 + NPR_025 * 0.25 + NPR_075 * 0.25) * 2.0/3.0
        # x = self.conv1(NPR)

        x = self.bn1(x)
        x = self.relu(x)
        # x = x * self.ca1(x)  # 应用通道注意力
        x = self.maxpool(x)

        x = self.layer1(x)
        # x = x * self.ca2(x) # 在关键特征层应用通道注意力
        x = self.layer2(x)
